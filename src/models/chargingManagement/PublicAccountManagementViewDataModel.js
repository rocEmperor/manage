...
export default {
  namespace: 'PublicAccountManagementViewDataModel',
  state:{
  ...
  effects:{
    ...
    *importSuccess ({ payload }, { call, put, select }) {
      let id = yield select(state => state.PublicAccountManagementViewDataModel.id);
      yield put({
    ...
    *createBill({ payload,callBack }, { call, put,select }){
      ...
      let ViewData = yield select(state => state.PublicAccountManagementViewDataModel);
      if (code == 20000) {
        ...
        yield put(routerRedux.push(`/publicAccountManagementView?period_id=${ViewData.id}&status=${ViewData.statusType}`));
      }
    },
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, search }) => {
        if (pathname === '/publicAccountManagementViewData') {
