...
export default{
  namespace: 'PublicAccountManagementViewModel',
  state:{
  ...
  effects:{
    ...
    *sharedPeriodBillList({ payload }, { call, put,select }){
      const params = yield select(state=>state.PublicAccountManagementViewModel.params);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, search }) => {
        if (pathname === '/publicAccountManagementView') {
