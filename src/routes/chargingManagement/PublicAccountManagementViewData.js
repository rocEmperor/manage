...
function PublicAccountManagementViewData (props){
  ...
  function pageChange(index){
    ...
    props.form.validateFields((err, values) => {
      dispatch({
        type:"PublicAccountManagementViewDataModel/sharedPeriodRecordList",
        payload:{
          period_id: id,
          page: index,
          rows: 10,
  ...
  function handleOk(e){
    ...
    dispatch({
      type:"PublicAccountManagementViewDataModel/createBill",
      payload:param
    })
  }
  ...
  function removeInfo(record){
    dispatch({
      type:"PublicAccountManagementViewDataModel/recordDelete",
      payload:{
        "id": record.id
      }
    })
  ...
  function hideModalVisible() {
    ...
    dispatch({
      type: 'PublicAccountManagementViewDataModel/importSuccess'
    })
  }
  /**
   * 批量导入中的下载模板
  ...
  function downFiles() {
    dispatch({
      type: 'PublicAccountManagementViewDataModel/downFiles',
      payload: {
        "community_id": getCommunityId()
      },callback(data){
        download(data);
  ...
  const columns = [{
    ...
    width:'170',
    render: (text, record) => {
      let iLink = `addWaterMeter?period_id=${id}&id=${record.id}&type=edit`;
      return(
        <div>
  ...
  let link12 = `/addWaterMeter?period_id=${id}`;
  let link13 = `/publicAccountManagementView?period_id=${id}&status=${statusType}`
  return (
    <div>
      <Breadcrumb separator=">">
        <Breadcrumb.Item>收费管理</Breadcrumb.Item>
        <Breadcrumb.Item><a href="#/billManagement">公摊账期管理</a></Breadcrumb.Item>
...
function mapStateToProps(state) {
  return {
    ...state.PublicAccountManagementViewDataModel,
  }
}
export default connect(mapStateToProps)(Form.create()(PublicAccountManagementViewData));
