...
function AddWaterMeter(props){
  ...
  function handleConfirmAdd(){
    props.form.validateFields((errors, values)=>{
      ...
      if(ID){
        if(amountMoney!=null){
          dispatch({
            type:"AddWaterMeterModel/recordEdit",
            payload:{
        ...
        if(amountMoney!=null){
          dispatch({
            type:"AddWaterMeterModel/recordAdd",
            payload:{
              'community_id':getCommunityId(),
              amount:amountMoney,
              current_num:values.current_num,
  ...
  function handleChange(e,value,recordNumber){
    if(e == "shared_type"){
      dispatch({
        type: 'AddWaterMeterModel/sharedPeriodList',
      ...
      data.map((item)=>{
        if(item.name == value){
          dispatch({
            type: 'AddWaterMeterModel/concat',
            payload: {values:value,id:item.id},
          });
          dispatch({
            type: 'AddWaterMeterModel/recordNumber',
  ...
  function onBlurUserName(value){
    ...
    dispatch({
      type: 'AddWaterMeterModel/getRecordMoney',
      payload: param1,
    });
  }
  return(
    <div>
      <Breadcrumb separator=">">
        ...
        <Breadcrumb.Item>
          <Link to={`/billManagementViewData`}>
            录入数据
          </Link>
        </Breadcrumb.Item>
        <Breadcrumb.Item>
...
function mapStateToProps(state) {
  return {
    ...state.AddWaterMeterModel,
  }
}
export default connect(mapStateToProps)(Form.create()(AddWaterMeter));
