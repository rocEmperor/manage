...
let hasSubmit = false;
function ProjectManagementAdd(props){
  ...
  function handleChange(value){
    dispatch({
      type:'ProjectManagementAddModel/concat',
      payload:{
        type:value,
      }
    })
  ...
  function handleConfirmAdd(){
    form.validateFields((err,values)=>{
      ...
      if (!hasSubmit) {
        ...
        let func = id ? 'ProjectManagementAddModel/sharedEdit' : 'ProjectManagementAddModel/sharedAdd';
        dispatch({
          type: func,
          payload: payload,
          callback: () => {
            ...
            if (Object.hasOwnProperty.call(queryList, 'curTabPaneKey')) {
              window.location.hash = `#projectManagement?curTabPaneKey=${queryList.curTabPaneKey}`
            } else {
              window.location.hash = '#projectManagement'
            }
          },
          err: () => {
            hasSubmit = false;
  ...
  return (
    <div>
      <Breadcrumb separator=">">
        ...
        <Breadcrumb.Item><a onClick={handleBack}>公摊项目管理</a></Breadcrumb.Item>
...
function mapStatusToProps(state){
  return {
    ...state.ProjectManagementAddModel,
    loading:state.loading.models.ProjectManagementAddModel,
  }
}
export default connect (mapStatusToProps)(Form.create()(ProjectManagementAdd))
